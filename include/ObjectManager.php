...
class ObjectManager {
    ...
    public function createHostTemplate( $templateHostName,$templateHostDescription="", $exportConfiguration = FALSE ){
        global $lilac;
        $error = "";
		$success = "";
		$code =0;
    ...
    public function createHostGroup( $hostGroupName, &$error = "", &$success = "", $exportConfiguration = FALSE ){
        global $lilac;
        $hostGroup = NULL;
		...
		if(empty($error)) {	
			try {
				...
				foreach($services as $service) {
					...
					$template = NagiosServiceTemplatePeer::getByName($service);
					$newInheritance->setNagiosServiceTemplateRelatedByTargetTemplate($template);
					$newInheritance->save();
					$success .= "Service Template ".$service->template." added to service $service->name \n";
					...
					foreach($service->parameters as $param) {
						...
						$param->save();
						$success .= "Command Parameter ".$param." added to $service\n";
					}
				}
	...
	##########################################ADD
	/* LILAC - add Command */
    public function addCommand($commandName,$commandLine,$commandDescription=""){
		$error = "";
		$success = "";
		$code=0;
		try{
    ...
    public function addServiceToTemplate( $templateName, $services, $template = NULL, $exportConfiguration = FALSE ){
        ...
        if( $template == NULL ){
            ...
            if(!$template) {
                $error .= "Host $templateName doesn't exist\n";
			}
        }
		...
		if(empty($error)) {	
			try {
				...
				foreach($services as $key => $service) {
					...
					$tempService->setNagiosHostTemplate($template);
					$tempService->save();
    ...
    public function deleteServiceTemplate($templateName){
		...
		try{
			...
			if(!$targetTemplate) {
				...
				$success .= "The template '".$templateName."' deleted.\n";
			}
		}catch(Exception $e) {
			$code=1;
			$error .= $e->getMessage()."\n";
